...
@partial(jnp.vectorize, excluded=(4, 5, 6, 7), signature="(m),(m),(m),(m,m)->(d),(d,d)")
def _predicted_observation(x_k_1, x_k, filtered_mean, filtered_covariance, transition_function,
                           observation_function, transition_covariance, observation_covariance):
    F = jacfwd(transition_function, 0)(x_k_1)
    cov = jnp.dot(F, jnp.dot(filtered_covariance, F.T)) + transition_covariance
    mean = transition_function(x_k_1)
    mean = mean + jnp.dot(F, filtered_mean - x_k_1)

    H = jacfwd(observation_function, 0)(x_k)

    obs_mean = observation_function(x_k) + jnp.dot(H, mean - x_k)

    obs_covariance = jnp.dot(H, jnp.dot(cov, H.T)) + observation_covariance

    return obs_mean, obs_covariance
...
def filter_routine(initial_state: MVNormalParameters,
    ...
    # Log-likelihood
    filtered_means_for_obs = jnp.concatenate([initial_state.mean[None, :], filtered_means[:-1]], axis=0)
    filtered_covs_for_obs = jnp.concatenate([initial_state.cov[None, ...], filtered_covariances[:-1]], axis=0)

    obs_means, obs_covs = _predicted_observation(x_k_1_s, linearization_points, filtered_means_for_obs,
                                                 filtered_covs_for_obs, transition_function, observation_function,
                                                 transition_covariance, observation_covariance)

    logprobs = jnp.vectorize(mvn.logpdf, signature="(m),(m),(m,m)->()")(observations, obs_means, obs_covs)

    return vmap(MVNormalParameters)(filtered_means, filtered_covariances), jnp.sum(logprobs)
